...
- [ ] **TODO — Small history drawer (next)** — API exists, UI not yet. Add a slide-over in `FacultyGradeEncodingPage` (and optionally My Students drawer) that fetches `GET /api/grades/audits?subjectCode=&studentUsername=&branch=` and shows timeline: `period old→new • action • performedBy • performedAt • note`. Keep API-only for now as requested; drawer is next. Tracked here.
