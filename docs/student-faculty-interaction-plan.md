...
| #34 | decision, then days or hours | scope first | Message section: in-app messaging, notification reuse, or remove | decided, no dead button | done 2026-09-12 as option B (notification reuse, owner decision) |
| #35 | decision, then days or ~1 day | scope first | Upload materials: Cloudinary, link-only, or remove | decided, students see materials if built | done 2026-09-12 as Cloudinary primary + link fallback (owner decision) |
| #36 | low | ~1 hour | Announce quiz duplicates Tasks (type Quiz exists), In/Out log has no data source | no coming-soon toasts left in FacultyStudentsPage | done 2026-09-12 (quiz handoff; In/Out restored as disabled Soon placeholder per owner — still no data source) |
...
- #36 follow-up (2026-09-12, owner request): In/Out log restored as a disabled Soon placeholder (repo Soon-badge pattern, `aria-disabled`, no toast, tooltip notes the missing data source) instead of staying deleted. Still no backend or data source behind it — it becomes real work only if entry/exit tracking appears. Verified in browser: badge renders, click is inert (0 toasts).
- #34 (2026-09-12): owner chose option B (notification reuse). Message section button in the roster section footer now opens a composer Modal (title + body, section fixed) that sends through the existing `POST /api/notifications` fan-out — no new collection, one-way faculty to section, read state persists in the student bell inbox. No dead button left. Verified: `npx tsc --noEmit` clean, `npm run lint` clean in touched files. API: sent to CS 208 section (3 students), student inbox received unread with correct fromName, mark-read persisted, student send got 403. Browser as `m.reyes`: composer opened for CS 208, Send closed the modal, message landed unread in `juan.santos` inbox. Test docs created and deleted: 6 `notifications` docs (`VERIFY34-MSG ...` + `VERIFY34-UI ...`, 0 remain).
- #35 (2026-09-12): owner chose Cloudinary as primary with link-only fallback. New `materials` collection (one doc per posting: branch/subjectCode/term/title/kind/url + Cloudinary publicId/resourceType/bytes/format for files). Files upload direct browser→Cloudinary with a server-signed signature (`POST /api/materials/sign`, secret never leaves the server) into the portal's own folder tree `aics-portal/materials/<branch>/<SUBJECT>/`; records via `POST /api/materials` (10 MB cap, type allowlist, oversized/disallowed orphans destroyed); `DELETE /api/materials/[id]` destroys the asset (CDN-invalidated) best-effort; `GET /api/materials` role-aware (students: own current-term subjects, faculty: taught load). Faculty manager modal in the roster drawer (Upload/Link tabs, per-subject list with Open/Delete); students get an Academics → Materials mini-tab. Without `CLOUDINARY_*` env, signing returns 503 and the Upload tab yields to links (verified). Creds live only in deploy/local env (see `example.env.local` placeholders) — never in code, logs, or the PR. Verified: `npx tsc --noEmit` clean, `npm run lint` clean in touched files, `npm run build` passes. API: link post → student sees; sign → curl upload → record → student sees file+link; guards (student post 403, bad URL 400, student delete 403, unsigned env 503); delete destroys asset (delivery 200 before, 404 after). Browser as `m.reyes`: modal posted link + uploaded txt (list showed File/TXT + Link), student tab showed both with sizes; both deleted via UI, list empty. Test docs created and deleted: `materials` docs (`VERIFY35-*`, 0 remain) + Cloudinary assets in the CS-208 test folder (folder empty in both resource types after). Two real bugs found and fixed during verification: missing `format` on raw uploads (derive from filename) and CDN caching after destroy (`invalidate=true`).
