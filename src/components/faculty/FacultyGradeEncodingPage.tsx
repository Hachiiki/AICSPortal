                  ...
                  {filtered.map(row=>{
                    ...
                    return (
                      <tr key={row._key} className={`border-b border-slate-100 last:border-b-0 ${row.dirty?'bg-amber-50/60':''}`}>
                        ...
                        {showPrelim && <td className="px-2 py-2.5 text-center"><input disabled={period==='all'} value={period==='all' && !row.prelim ? 'INC' : row.prelim} onChange={e=>onGradeInput(row._key,'prelim',e.target.value)} placeholder={period==='all'?'INC':'—'} className={`w-[72px] h-8 px-2 text-center mono text-sm rounded-lg border ${period==='all'?'bg-slate-100 border-slate-200 text-slate-500 cursor-not-allowed':'bg-white border-slate-200 focus:border-blue-500 focus:ring-1'}`} /></td>}
                        {showMidterm && <td className="px-2 py-2.5 text-center"><input disabled={period==='all'} value={period==='all' && !row.midterm ? 'INC' : row.midterm} onChange={e=>onGradeInput(row._key,'midterm',e.target.value)} placeholder={period==='all'?'INC':'—'} className={`w-[72px] h-8 px-2 text-center mono text-sm rounded-lg border ${period==='all'?'bg-slate-100 border-slate-200 text-slate-500 cursor-not-allowed':'bg-white border-slate-200 focus:border-blue-500 focus:ring-1'}`} /></td>}
                        {showFinals && <td className="px-2 py-2.5 text-center"><input disabled={period==='all'} value={period==='all' && !row.finals ? 'INC' : row.finals} onChange={e=>onGradeInput(row._key,'finals',e.target.value)} placeholder={period==='all'?'INC':'—'} className={`w-[72px] h-8 px-2 text-center mono text-sm rounded-lg border ${period==='all'?'bg-slate-100 border-slate-200 text-slate-500 cursor-not-allowed':'bg-white border-slate-200 focus:border-blue-500 focus:ring-1'}`} /></td>}
