...
export function FacultyGradeEncodingPage({ student, onNavigate, onLogout, events, professors, tasks, facultyData, facultyLoading }: Props) {
  ...
  // Build sections like StudentsPage — group by code|ay|sem — from MongoDB subjects collection
  ...
  const onGradeInput=useCallback((key:string,field:string,val:string)=>{
    ...
    setRows(prev=>prev.map(r=> {
      ...
      if(r.locked || r.gradeStatus !== 'draft') return r
      const next = {...r,[field]:val}
      // recompute finalGrade when prelim/mid/finals change and we are in finals view or all edits should keep fg in sync
      const fg = computedFinal(field==='prelim'?val:r.prelim, field==='midterm'?val:r.midterm, field==='finals'?val:r.finals)
      // keep finalGrade in sync only if finals period or if finalGrade was empty
      if(field !== 'finalGrade') {
        // don't overwrite manually edited finalGrade unless prelim/mid/finals changed to INC?
        // For simplicity, keep finalGrade as derived when prelim/mid/finals are all numeric/INC
        // But don't auto-fill finalGrade field — let table's readOnly handle display. We store empty unless finals entered.
        // However for API we need to store finalGrade if computed and finals present.
      }
  ...
  // --- MongoDB wiring: Save per-period drafts ---
  const handleSavePeriod = useCallback(async () => {
    ...
    const targets = filtered.filter(r=> r.dirty && (r as any)[field] !== (r.original as any)[field] && !r.locked && r.gradeStatus==='draft')
    ...
    try {
      const updates = targets.map(r=>{
        ...
        // Only persist the period field + recomputed finalGrade/remarks if finals period, otherwise just period field
        const payload: any = { studentUsername: r.studentUsername, subjectCode: r.subjectCode, branch, [field]: (r as any)[field] }
        // Always persist finalGrade if computable (covers INC propagation)
        if(fg) {
          payload.finalGrade = fg
          payload.remarks = remarks || r.remarks
        } else if(field==='finals' && !(r as any)[field]) {
          // finals cleared -> clear finalGrade
          payload.finalGrade = ''
        }
        return payload
      })
      ...
      // Optimistically update original to match saved values and clear dirty
      setRows(prev=>prev.map(r=> {
      ...
      toast.success(`Saved ${targets.length} ${period} draft(s) to MongoDB — ${activeSection==='all'?`All sections (${filtered.length})`:`${activeSection} (${filtered.length})`}`)
    } catch(e:any){
  ...
  // --- MongoDB wiring: Submit per-period (blanks→INC, draft→submitted) ---
  const handleSubmitConfirm = useCallback(async () => {
    ...
    setSubmitting(true)
    try {
      // 1) Fill blanks with INC for the submitPeriod scope (and mark dirty)
      const fieldMap:any = { prelim:'prelim', midterm:'midterm', finals:'finals' }
      if(submitPeriod!=='all'){
        const f = fieldMap[submitPeriod]
        // locally patch blanks to INC for draft rows
        const patchedRows = rows.map(r=>{
          const inScope = vis.some(v=>v._key===r._key)
          if(!inScope) return r
          if(r.gradeStatus!=='draft' || r.locked) return r
          if(!(r as any)[f] || (r as any)[f]===''){
            const fgInc = submitPeriod==='finals' ? 'INC' : r.finalGrade
            return {...r, [f]:'INC', finalGrade: f==='finals'?'INC':r.finalGrade, remarks: f==='finals'?'INC':r.remarks, dirty:true }
          }
          return r
        })
        // Persist those INC fills + any other dirty grades first
        const dirtyForPatch = patchedRows.filter(r=> vis.some(v=>v._key===r._key) && r.dirty && r.gradeStatus==='draft' && !r.locked)
        ...
        setRows(patchedRows)
        // 2) Now flip gradeStatus draft→submitted per distinct subject+term via API
        const distinct = new Map<string,{subjectCode:string, academicYear:string, semester:string}>()
        vis.forEach(r=>{ if(r.gradeStatus==='draft' && !r.locked){ const k=`${r.subjectCode}|${r.academicYear}|${r.semester}`; if(!distinct.has(k)) distinct.set(k,{subjectCode:r.subjectCode, academicYear:r.academicYear, semester:r.semester}) }})
        ...
        // Optimistically mark those rows submitted+locked
        setRows(prev=>prev.map(r=>{
          const inScope = vis.some(v=>v._key===r._key)
          if(!inScope || r.gradeStatus!=='draft' || r.locked) return r
          // only mark as submitted if that period now has a value
        ...
        toast.success(`Submitted ${totalSubmitted || vis.filter(r=>r.gradeStatus==='draft').length} records (${submitPeriod}) — MongoDB draft → submitted`)
        setShowSubmitModal(false)
      } else {
        // submitPeriod === 'all' — fill all three blanks to INC where needed
        const patchedRows = rows.map(r=>{
          const inScope = vis.some(v=>v._key===r._key)
          if(!inScope || r.gradeStatus!=='draft' || r.locked) return r
          let nr = {...r}
        ...
        setRows(prev=>prev.map(r=> vis.some(v=>v._key===r._key) && r.gradeStatus==='draft' && !r.locked ? {...r, gradeStatus:'submitted', locked:true, dirty:false, original:{ prelim:r.prelim, midterm:r.midterm, finals:r.finals, finalGrade:r.finalGrade }} : r))
        toast.success(`Submitted ${totalSubmitted || vis.length} records (all periods) — MongoDB`)
        setShowSubmitModal(false)
      }
    } catch(e:any){
  ...
  return (
    <div className="min-h-dvh bg-slate-50 font-sans">
        ...
        <main className="px-4 sm:px-6 lg:px-8 py-6 lg:py-8 space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">{faculty.semester} • AY {faculty.academicYear} — Prelim / Midterm / Finals → final auto-computed <span className="text-xs text-slate-400 ml-1">(MongoDB: {branch} • {filtered.length}/{rows.length} rows)</span></p>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-slate-100 bg-slate-50/50">
              ...
              <div className="flex items-center gap-2">
                <span className="hidden sm:inline-flex items-center gap-1.5 text-xs text-amber-700 bg-[#fffbeb] border border-amber-100 rounded-full px-2.5 py-1"><Info className="w-3.5 h-3.5"/> Final • auto-computed • MongoDB-persisted</span>
                <button onClick={()=>toast.info('Auto-compute: Final = Prelim*0.3+Mid*0.3+Finals*0.4 — saved to MongoDB on Save')} className="px-3 py-1.5 rounded-lg border border-blue-200 bg-blue-50 text-xs font-semibold text-blue-700 hover:bg-blue-100 inline-flex items-center gap-1.5"><Calculator className="w-3.5 h-3.5"/> Auto-compute</button>
            ...
            {period!=='all' && (
              <div className="px-4 py-3 border-b border-slate-100 bg-white flex flex-wrap items-center justify-between gap-3">
                <p className="text-xs text-slate-600"><span className="font-semibold text-slate-900">{period==='prelim'?'Prelim only':period==='midterm'?'Midterm only':'Finals only'} — {activeSection==='all'?`All sections (${filtered.length})`:`${activeSection} • ${filtered.length} students`} • per-period save → MongoDB</span></p>
                <div className="flex items-center gap-2">
            ...
            <div className="px-4 py-3 flex flex-wrap gap-3 items-center border-b border-slate-100">
              ...
              <span className="text-xs text-slate-500">{filtered.length} / {rows.length} records • {branch}</span>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.map(row=>{
                    ...
                    const statusBadge = row.gradeStatus==='submitted' ? 'bg-blue-50 text-blue-700 border-blue-200' : row.gradeStatus==='released' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-100 text-amber-800 border-amber-200'
                    return (
                      <tr key={row._key} className={`border-b border-slate-100 last:border-b-0 ${row.dirty?'bg-amber-50/60':''} ${isLocked?'opacity-75':''}`}>
                        ...
                        {period!=='all' && <td className="px-3 py-2.5 text-left text-[11px] text-slate-500">{faculty?.username || 'm.reyes'} • {row.gradeStatus} {row.dirty ? '• unsaved' : ''} • {branch}</td>}
                      </tr>
                    )
                  })}
                </tbody>
            ...
            {filtered.length===0 && <div className="py-10 text-center text-sm text-slate-500">No records for current filters • MongoDB live • {branch}</div>}
          </div>
        </main>
      </div>

      {/* Fill down modal — per section + per period only, marks dirty (Save persists to MongoDB) */}
      {showFillModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100">
              ...
              <p className="text-xs text-slate-500 mt-1">Fill this value for all visible students in <span className="font-medium text-slate-700">{activeSection==='all'?`All sections (${filtered.length})`:`${activeSection} • ${filtered.length} students`}</span> • {period==='prelim'?'Prelim':period==='midterm'?'Midterm':'Finals'} only. Then <b>Save</b> to persist to MongoDB.</p>
            </div>
            <div className="px-6 py-4 space-y-3">
              ...
              <p className="text-xs text-slate-500">Per-period — only fills the current tab’s column for the filtered section. Dirty rows need Save → MongoDB.</p>
            </div>
            <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-2">
              ...
              <button onClick={()=>{
                ...
                setRows(prev=>prev.map(r=> (r.subjectCode===activeSection && !r.locked && r.gradeStatus==='draft') ? {...r, [field]: val, dirty:true, gradeStatus:'draft'} : r))
                setShowFillModal(false); setFillValue(''); toast.success(`Filled ${val} for ${activeSection} • ${field} — click Save to write to MongoDB`)
      ...
      {/* Submit modal — scope-aware, blank → INC, draft→submitted via MongoDB */}
      {showSubmitModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/50" onClick={()=>setShowSubmitModal(false)} />
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100">
              <h3 className="font-bold">Submit grades to MongoDB?</h3>
              <p className="text-xs text-slate-500 mt-1">This submits the selected period (draft → submitted) via <code className="bg-slate-100 px-1 rounded">/api/grades/submit</code>. Students will see grades after admin <b>Releases</b>.</p>
              <div className="mt-3 p-3 rounded-xl bg-slate-50 border border-slate-200">
                <p className="text-xs font-semibold" id="submitScopeText">{activeSection==='all'?`All sections (${filtered.length})`: `${activeSection} • ${filtered.length} students`} — {submitPeriod==='all'?'All periods': submitPeriod==='prelim'?'Prelim only': submitPeriod==='midterm'?'Midterm only':'Finals only'} • {branch}</p>
                <p className="text-xs text-slate-500 mt-1" id="submitScopeDetail">
                  {(() => {
                    ...
                    if(need>0) return `${need} blank(s) will be set to INC on submit — ${drafts} graded + ${need} INC total. Will PATCH INC fills then POST submit.`
                    return `This will submit ${drafts} draft grades in this scope via MongoDB.`
                  })()}
                </p>
              </div>
              {(() => {
                ...
                return (
                  <div className="mt-3 p-3 rounded-xl bg-amber-50 border border-amber-200 flex gap-2">
                    ...
                    <div className="text-xs">
                      ...
                      <p className="text-amber-700 mt-1">{need} student(s) still have blank {submitPeriod}. Fill with a grade (0-100) or INC, or they will become INC in MongoDB.</p>
                    </div>
                  </div>
                )
              })()}
            ...
            <div className="px-6 py-4 space-y-3">
              ...
              <p className="text-xs text-slate-500">Branch: {branch} • {faculty?.username} • MongoDB subjects collection</p>
            </div>
            <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-2">
